  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        `;
        containerRef.current.id = "desmos-graph-container";
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const calculator = Desmos.ScientificCalculator(containerRef.current, {
          keypad: true,
          settingsMenu: false,
          border: false,
          expressions: false,
        ...
        console.error("Failed to load Desmos Scientific:", err);
      }
    }
